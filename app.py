...
# Reads the API key from a system environment variable.
# This is much more secure than pasting the key directly into the code.
...
# Ensures that even for a "Not Found" error, the API returns JSON.
@app.errorhandler(404)
def not_found(error):
    return jsonify({"error": "Not Found"}), 404

# --- API ENDPOINTS (UPGRADED WITH VALIDATION) ---
...
# --- THE "SMART" AI ENDPOINT (UPGRADED WITH ERROR CHECKS) ---
...
def get_summary(patient_id):
    ...
    try:
        ...
        # Catch any potential errors from the OpenAI API call
        return jsonify({"error": f"An error occurred with the AI service: {str(e)}"}), 500

# --- PROFESSIONAL DATABASE SETUP COMMAND ---
